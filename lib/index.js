...
var getSchemaFromRelationships = function (relationships, objectName) {
  ...
  if (relationships.type !== 'object') {
    throw new Error("relationships must be an object type.");
  }

  var properties = relationships.properties ? relationships.properties : attributes;

  _.forEach(properties, function(property, key) {
    var type, schemaRef, identifier, propType;

    if (!~['array', 'object'].indexOf(property.type)) {
      throw new Error("every relationship must be an object or array type.");
    }

    var data = _.get(property, 'properties.data');
    // if it's of type array, get the items property
    var linkObj = _.get(data, 'items', data);
    if (linkObj) {
      if (isPropertyWithRef(linkObj)) {
        propType = getReferenceType(linkObj);
        identifier = definitions[propType];
      } else {
        propType = key;
        identifier = linkObj;
      }

      if (identifier.type !== 'object' && _.has(identifier, 'properties')) {
        throw new Error(`The ${objectName} definition's ${key} relationship must reference a resource identifier.`);
      }

      if (!_.has(identifier, 'properties.type') && !_.has(identifier, 'properties.id')) {
      ...
      type = getRelationshipPropertyType(identifier, propType);
      schemaRef = getMongooseReference(identifier, type);
      schema[key] = data.items ? [schemaRef] : schemaRef;
    }
  });
...
var getSchemaFromResourceObject = function (resource, objectName) {
  ...
  if (properties.type) {
    _.extend(schema, getSchemaFromProperty(properties.type, 'type', [], objectName, {}));
  }
