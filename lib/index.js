...
var processRef = function (property, objectName, schema, key, required) {
  ...
  if (propType !== objectName) {
    ...
    if (~['array', 'object'].indexOf(object.type)) {
      ...
      schema[key] = property['items'] || object.type === 'array' ? [schema] : schema;
    } else {
...
var getSchemaFromAttributes = function (attributes, type) {
  var schema = {};
  ...
  _.forEach(properties, function (property, key) {
    var schemaProperty = getSchemaFromProperty(property, key, required, type, properties);
    schema = _.extend(schema, schemaProperty);
  });

  return schema;
};

var getSchemaFromRelationships = function (objectName, relationships, required) {
  var schema = {};

  _.forEach(relationships, function(relationship, key) {
    // Links object can be ignored
    if (relationship.data) {
      // TODO process relationship data...
    }
  });
}
var getSchemaFromResourceObject = function (objectName, resource, required) {
  if (!_.has(resource, 'type') && !_.has(resource, 'id')) {
    throw new Error('Resource objects must contain both an id and type property');
  }

  var invalidKeys = _.difference(_.keys(resource), allowedResourceKeys);
  ...
  if (resource.attributes) {
    var schemaProperties = getSchemaFromAttributes(resource.attributes, objectName);
...
var getSchema = function (objectName, fullObject, isJsonApi) {
  ...
  if (isJsonApi) {
    var schemaProperties = getSchemaFromResourceObject(objectName, properties, required);
    _.extend(schema, schemaProperties);
  } else {
    _.forEach(properties, function (property, key) {
      var schemaProperty = getSchemaFromProperty(property, key, required, objectName, properties);
      _.extend(schema, schemaProperty);
    });
  }
