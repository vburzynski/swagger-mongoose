...
var processAttributesObject = function (attributes, type) {
  var props = {};
  var required = attributes.required || [];
  var object = attributes.properties ? attributes.properties : object;

  _.forEach(object, function (property, key) {
    var schemaProperty = getSchemaProperty(property, key, required, type, object);
...
var processResourceObject = function (objectName, resourceObject, required) {
  if (!_.includes(required, 'type') && !_.includes(required, 'id')) {
    throw new Error('Resource objects must contain both an id and type property');
  }

  var invalidKeys = _.difference(_.keys(resourceObject), allowedResourceKeys);
  if (invalidKeys.length > 0) {
    throw new Error('Resource resourceObject contains properties not within the JSON API Specification');
  }

  var props = {};

  var schemaProperty = getSchemaProperty(resourceObject.id, 'id', required, objectName, resourceObject);
  props = _.extend(props, schemaProperty);

  if (resourceObject.attributes) {
    var schemaProperties = processAttributesObject(resourceObject.attributes, objectName);
    _.extend(props, schemaProperties);
  }

  // TODO: Process relationship object
  // TODO: Process links object
  // TODO: Process meta object
  // TODO: Process error objects

  return props;
};

var getSchema = function (objectName, fullObject, useJsonApiCheck) {
  var props = {};
  var required = fullObject.required || [];
  var object = fullObject['properties'] ? fullObject['properties'] : fullObject;

  useJsonApiCheck = useJsonApiCheck !== false;

  if (useJsonApiCheck && xSwaggerMongoose.isJsonApi) {
    // TODO Detect other types of objects here such as error objects
    var schemaProperties = processResourceObject(objectName, object, required);
    props = _.extend(props, schemaProperties);
  } else {
    _.forEach(object, function (property, key) {
...
var getSchemaProperty = function(property, key, required, objectName, object) {
  ...
  if (isMongooseProperty(property)) {
    ...
    props[key] = getSchema(key, property, false);
  }
...
var processMongooseDefinition = function(key, customOptions) {
  if (customOptions) {
    if (customOptions['json-api']) {
      xSwaggerMongoose.isJsonApi = customOptions['json-api'];
    }
