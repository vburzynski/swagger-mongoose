...
var processResourceObject = function (objectName, resourceObject, required) {
  if (!_.has(required, 'type') && !_.has(required, 'id')) {
  ...
  if (type !== objectName) {
    console.warn(`Type property does not match the object name specified in the JSON Definitions (${type} != ${objectName}).`);
  }

  var props = {};

  var schemaProperty = getSchemaProperty(resourceObject.id, 'id', required, type, resourceObject);
  props = _.extend(props, schemaProperty);

  if (resourceObject.attributes) {
    var schemaProperties = processAttributesObject(resourceObject.attributes, type);
