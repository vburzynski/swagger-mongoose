...
var processRef = function (property, objectName, props, key, required) {
  ...
  if (propType !== objectName) {
    ...
    if (~['array', 'object'].indexOf(object.type)) {
      // TODO: This might only work if the referenced item was already processed.
      var isJsonApi = hasJsonApiOption(propType);
      var schema = getSchema(propType, object['properties'] ? object['properties'] : object, isJsonApi);
...
var processResourceObject = function (objectName, resourceObject, required) {
  if (!_.includes(resourceObject, 'type') && !_.includes(resourceObject, 'id')) {
